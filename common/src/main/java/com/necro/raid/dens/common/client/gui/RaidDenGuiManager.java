...
public class RaidDenGuiManager {
    ...
    public static void render(GuiGraphics guiGraphics, int maxX, int maxY) {
        if (hasPartyOverlay()) RAID_OVERLAY.render(guiGraphics, maxX, maxY);
        if (hasOverlayQueue()) OVERLAY_QUEUE.getFirst().render(guiGraphics, maxX, maxY);
    }
